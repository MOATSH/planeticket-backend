...
public class PredictPriceInfoServiceImpl extends ServiceImpl<PredictPriceInfoMapper, PredictPriceInfo> implements PredictPriceInfoService {
    ...
    public FlightRecommendTimeRespDTO recommendTime(FlightRecommendTimeReqDTO requestParam) {
        if (Boolean.TRUE.equals(stringRedisTemplate.hasKey(RedisCacheConstant.FLIGHT_RECOMMEND_TIME + requestParam))) {
            return JSONUtil.toBean(stringRedisTemplate.opsForValue().get(RedisCacheConstant.FLIGHT_RECOMMEND_TIME + requestParam), FlightRecommendTimeRespDTO.class);
        } else {
            //获取出发地和目的地的城市信息
            ...
            // 设置缓存
            stringRedisTemplate.opsForValue().set(RedisCacheConstant.FLIGHT_RECOMMEND_TIME + requestParam, JSONUtil.toJsonStr(respDTO), 30, TimeUnit.MINUTES);
